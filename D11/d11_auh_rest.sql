...
begin
  ...
  ords.create_privilege(
      p_name => 'd11.PRIVILEGE',
      p_role_name => 'd11.ROLE',
      p_label => 'd11 data',
      p_description => 'Provide access to d11 data');
  commit;
end;
/

--select id,name from user_ords_privileges where name = 'd11';

begin
 ords.create_privilege_mapping(
      p_privilege_name => 'd11.PRIVILEGE',
      p_pattern => '/d11/*');     
  commit;
end;
/
...
begin 
 oauth.create_client(
      ...
      p_privilege_names => 'd11.PRIVILEGE',
      p_support_email =>'pippo@pippo.com');
 commit;
end;
/

--select name, client_id,client_secret from user_ords_clients where name = 'client_d11';
...
begin 
 oauth.grant_client_role(
     'client_d11',
     'oracle.dbtools.role.autorest.WKSP_MA0.d11_VENDITA');
 commit;
end;
/

--select * from user_ords_client_roles where client_name = 'client_d11';
https://cnde8nae4maapcd-oramms.adb.eu-zurich-1.oraclecloudapps.com/ords/ma0/d11/vendita/
...
BEGIN
    apex_web_service.oauth_authenticate_credential(
        ...
        p_credential_static_id => 'client_d11');
        l_response := APEX_WEB_SERVICE.MAKE_REST_REQUEST(p_url => 'https://cnde8nae4maapcd-oramms.adb.eu-zurich-1.oraclecloudapps.com/ords/ma0/d11/vendita/',
                                                         p_http_method => 'GET',
